...
type Audit struct {
	...
	UpdatedTimestamp string `json:"updated_timestamp"` //current time in milliseconds as a string
}
...
func (t *AuditTrailChaincode) createAudit(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	*/
	...
	if err == nil {
		...
		fmt.Println("Exist audit will be override for hash: " + audit.AuditHash)
	}
...
func (t *AuditTrailChaincode) Query(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
	...
	if function == "GetAllAudits" {
		...
		bytes, err := stub.GetState(args[0]) // search chain by key
